 ...
 *
 * Options are every active user; the division / department / team dropdowns only
 * narrow what is listed — any active user stays selectable.
 *
...
export default function PeoplePicker({ value = [], onChange, disabled = false }) {
    ...
    const visibleUsers = useMemo(() => {
        ...
        return data.users.filter((u) => {
            if (divisionId && String(u.division_id) !== String(divisionId)) return false;
            if (departmentId && String(u.department_id) !== String(departmentId)) return false;
            if (teamId && String(u.team_id) !== String(teamId)) return false;
            if (q && !`${u.name} ${u.position || ''}`.toLowerCase().includes(q)) return false;
            return true;
        });
    }, [data, search, divisionId, departmentId, teamId]);
