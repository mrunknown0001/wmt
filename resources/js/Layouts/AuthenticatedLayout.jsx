...
const TagNavIcon = () => (
    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 7h.01M7 3h5a1.99 1.99 0 011.414.586l7 7a2 2 0 010 2.828l-5 5a2 2 0 01-2.828 0l-7-7A1.99 1.99 0 013 10V5a2 2 0 012-2z" />
    </svg>
);

const TimeCorrectionIcon = () => (
    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6v6l3.5 2M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
    </svg>
);
...
export default function AuthenticatedLayout({ children, title, contained = false }) {
    const { auth, flash, settings, unreadNotificationsCount, pendingApprovalsCount, pendingTimeCorrectionsCount } = usePage().props;
        ...
        <div className="flex flex-col h-full">
            ...
            <nav className={`flex-1 ${collapsed ? 'px-2' : 'px-3'} pt-4 pb-2 space-y-6 overflow-y-auto scrollbar-thin`}>
                <div className="space-y-0.5">
                    ...
                    {/* The people you are responsible for. Shown to whoever heads a
                        division, department or team — and to admins and executives,
                        for whom it renders the whole org chart. */}
                ...
                {hasPermission('view-projects') && (
                    <div className="space-y-0.5">
                        ...
                        {/* Tags cross projects, tasks and minutes, so they sit
                            with the work rather than under any one of them. */}
                        <NavLink href="/tags" icon={<TagNavIcon />} active={isActive('/tags')} collapsed={collapsed}>
                            Tags
                        </NavLink>
                    </div>
                )}
                ...
                {/* Time corrections. Everybody can raise one from a task, so
                    everybody has a page; the badge only appears for the people
                    who have some to decide. */}
                <div className="space-y-0.5">
                    <NavLink
                        href="/time-corrections"
                        icon={<TimeCorrectionIcon />}
                        active={isActive('/time-corrections')}
                        badge={pendingTimeCorrectionsCount || null}
                        collapsed={collapsed}
                    >
                        Time Corrections
                    </NavLink>
